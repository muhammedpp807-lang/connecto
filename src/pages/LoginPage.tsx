...
import { Lock, Mail, Eye, EyeOff, ArrowRight, Loader2, UserCheck, Shield } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('success', 'Welcome back to Connecto!');
      navigate(from, { replace: true });
    } catch (err: unknown) {
      let msg = err instanceof Error ? err.message : 'Invalid credentials. Please try again.';
      if (msg.includes('auth/configuration-not-found') || msg.includes('auth/user-not-found') || msg.includes('auth/invalid-credential')) {
        msg = 'No account found with these credentials. Please check your details or create a free account below.';
  ...
  const handleQuickDemoLogin = async (username: string, pass: string) => {
    ...
    setErrorMsg('');
    try {
      await login({ emailOrUsername: username, password: pass });
      showToast('success', `Signed in as ${username}!`);
      navigate(from, { replace: true });
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 bg-slate-50 dark:bg-[#0a0c12] text-slate-900 dark:text-slate-100 transition-colors">
      <SEO title="Sign In – Connecto" description="Sign in to your Connecto account to access real-time messaging." />
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center mb-8">
        ...
        <p className="mt-2 text-xs sm:text-sm text-slate-600 dark:text-slate-400">
          Enter your credentials to access your active conversations
        </p>
      </div>
        ...
        <div className="bg-white dark:bg-[#161b22] py-8 px-6 sm:px-10 shadow-2xl border border-slate-200 dark:border-[#1e2530] rounded-3xl space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full mt-2 flex justify-center items-center gap-2 py-3 px-4 rounded-xl shadow-lg bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold transition disabled:opacity-50 cursor-pointer shadow-blue-900/20"
            >
              {loading ? (
                <>
